...
raw_deaths <- resp_body_json(resp, simplifyVector = TRUE) |>
  as_tibble() |>
  clean_names()
...
state_col  <- pick_first(c("state","jurisdiction","state_name","name"), raw_deaths)
date_col   <- pick_first(c("end_date","week_end","week_ending","submission_date","report_date","as_of","date"), raw_deaths)
deaths_col <- pick_first(c("covid_19_deaths","new_deaths","new_death","deaths","tot_death","total_deaths","death"), raw_deaths)

deaths <- raw_deaths |>
  transmute(
    state  = as.character(.data[[state_col]]),
    date   = parse_cdc_date(.data[[date_col]]),
    deaths = parse_number(.data[[deaths_col]])
  ) |>
